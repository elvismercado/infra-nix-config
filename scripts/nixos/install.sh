...
# ──────────────────────────────────────────────────────────────
# Provision the private sibling repo (nix-config-private)
# ──────────────────────────────────────────────────────────────
...
repo wasn't reachable during install. The flake requires a sibling repo
at this path (see public \`flake.nix\` \`inputs.private\`); this stub
lets the flake evaluate and rebuilds succeed.

The stub's \`hosts/<HOST>/user-settings.nix\` returns \`{ }\`, so the
NixOS/darwin modules fall back to their defaults (Europe/London time
zone, en-GB language) and emit a build-time \`lib.warn\` per host.
