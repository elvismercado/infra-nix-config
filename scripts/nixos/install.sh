...
# companion supports private-overlay editing/sync workflows. Normal evaluation
# consumes committed and pushed metadata through the authenticated GitHub
# input. The stub fallback is retained for a future local-input override, but
# does not currently replace that input; see TODO.md.
#
...
flake input does not consume this stub; see the public TODO.md before relying
on it for a no-access install.
