  ...
  config = lib.mkMerge [
    (lib.mkIf config.custom.hmLinuxAliases.enable {
      home.shellAliases = {
        ...
        switchupgrade = "bash ${workflowScriptArg} upgrade ${publicRepoArg} ${privateRepoArg} ${lib.escapeShellArg userSettings.hostname} linux";
        switchbuildalllinux = "bash ${workflowScriptArg} build-all ${publicRepoArg} linux";
