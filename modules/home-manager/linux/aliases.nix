  ...
  config = lib.mkMerge [
    (lib.mkIf config.custom.hmLinuxAliases.enable {
      home.shellAliases = {
        ...
        switch = "cd ${publicRepo} && ${bumpPrivate} && sudo nixos-rebuild switch --flake .#${userSettings.hostname}";
        switchverbose = "cd ${config.home.homeDirectory}/${userSettings.repoPath} && sudo nixos-rebuild switch --flake .#${userSettings.hostname} --show-trace --print-build-logs -L -v";
