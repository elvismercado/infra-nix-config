...
KWallet is enabled so applications can persist credentials securely. Complete
this one-time setup after deploying the configuration:
...
3. Select classic password encryption and set the wallet password to the same
   ...
   repository.
4. Re-authenticate applications if prompted so they can populate the wallet.
