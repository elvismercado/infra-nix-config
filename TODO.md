...
- [ ] **Silence `os-prober` errors during `nixos-install` on single-OS hosts.** LULA's install log emitted `ERROR: mkdir /var/lock/dmraid` and `modprobe: can't change directory to '/lib/modules': No such file or directory` from `os-prober` running inside the `grub-install` chroot. Harmless (os-prober wouldn't find anything on a single-OS box anyway), but noisy. Options: (a) expose `useOSProber` as a `custom.sysNixGrub.useOSProber` option (default `true` to preserve dual-boot behaviour on FENNEC) and set `false` on LULA; (b) auto-detect single-disk single-OS installs and disable. Lives in [modules/systems/nixos/bootloader/grub.nix](modules/systems/nixos/bootloader/grub.nix#L80).
