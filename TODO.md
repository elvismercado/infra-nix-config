...
- [x] **Window borders on maximized windows.** ✅ Done 2026-05-19 — pinned `programs.plasma.kwin.borderlessMaximizedWindows = false;` in [modules/home-manager/linux/plasma/lula.nix](modules/home-manager/linux/plasma/lula.nix). plasma-manager exposes this directly (no `configFile` needed); writes `kwinrc [Windows] BorderlessMaximizedWindows=false`. Already the KDE default; pinned defensively against future theme/preset flips. LULA-only for now — promote to common if FENNEC/JIN want the same defensive pin.
