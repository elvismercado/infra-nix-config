# infra-nix-config — Improvement Backlog
...
- [x] **Migrate `inputs.private` away from relative `git+file:` before Nix removes it.** ✅ Done 2026-05-19 — migrated to `github:elvismercado/infra-nix-config-private` (private repo, tarball API). Auth is per-invocation via `--option access-tokens "github.com=$(gh auth token)"` spliced into every `switch*` alias in [modules/home-manager/{all,linux,darwin}/aliases.nix](modules/home-manager/all/aliases.nix); prerequisite is `gh auth login` once per user per host. `bumpPrivate` was dropped from `switch` / `switchcheck` since the fetch is now network-bound; `switchbumpprivate` remains the explicit "I edited the private overlay" command. On-disk sibling next to `infra-nix-config` is no longer required for rebuilds (only for editing the overlay). See [flake.nix](flake.nix) header comment for the `path:` / `git+https:` / `git+ssh:` failure modes that ruled them out.
...
- [x] **Global UI scaling 110–125%.** ✅ Done 2026-05-19 — declarative scaling is not viable on Plasma 6.6 Wayland (per-output scale lives in `~/.config/kwinoutputconfig.json`; KWin generates persistent output UUIDs and associates them using EDID identity, MST path, and connector information; plasma-manager [#243](https://github.com/nix-community/plasma-manager/issues/243) has no option for it and the X11-era `kdeglobals [KScreen] ScreenScaleFactors` is ignored by KWin Wayland for window placement). Resolved instead with a two-layer approach on LULA: (1) declarative `programs.plasma.fonts` set to Plasma 6 defaults +2pt across all six categories in [modules/home-manager/linux/plasma/lula.nix](modules/home-manager/linux/plasma/lula.nix); (2) optional stronger magnification via a one-time GUI step (System Settings → Display & Monitor → Global Scale) documented under "Display scaling" in [hosts/LULA/README.md](hosts/LULA/README.md). KWin persists the scale across reboots.
...
- [x] **Load private metadata through `inputs.private`.** Done 2026-08-07 - [flake/metadata.nix](flake/metadata.nix) now accepts the explicit private source used by the flake output and Syncthing peer projection. Private-only peers are available during normal store-backed evaluation. Offline local-stub substitution remains separate below.
- [ ] **The NixOS installer's offline private stub does not satisfy the current GitHub flake input.** [flake.nix](flake.nix) fetches `private` through the authenticated `github:` scheme, while [scripts/nixos/install.sh](scripts/nixos/install.sh) provisions a local sibling or stub and invokes `nixos-install` without overriding that input. Decide whether install must require GitHub access and pass `access-tokens`, or preserve offline installs by overriding `private` with the local sibling/stub. Until then, do not claim the stub guarantees a successful no-access install.
