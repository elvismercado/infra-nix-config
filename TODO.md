...
- [x] **Single-window mode in Dolphin + sensible defaults.** ✅ Done 2026-05-19 — added `custom.hmPlasmaCommon.dolphin.enable` (default `false`) in [modules/home-manager/linux/plasma/common.nix](modules/home-manager/linux/plasma/common.nix); enabled on LULA. Writes `dolphinrc [General]` keys `BrowseThroughArchives`, `ShowFullPath`, `ShowToolTips`, `ShowStatusBar`, `OpenExternallyCalledFolderInNewTab=false`, `RememberOpenedTabs=false`, plus `[KFileDialog Settings] View Style=DetailsView`. Note: Dolphin's tabs feature itself has no declarative kill switch; the `OpenExternallyCalledFolderInNewTab=false` key is the closest approximation (externally-opened folders reuse an existing window instead of spawning a new tab).
