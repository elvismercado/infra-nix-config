...
- [ ] **Re-enable RustDesk on Linux hosts once nixpkgs builds again.** Disabled 2026-06-05 on JIN, FENNEC, LULA — `pkgs.rustdesk` 1.4.6 fails to build on Linux ([nixpkgs#527155](https://github.com/NixOS/nixpkgs/issues/527155): non-deterministic cargo-vendor FOD hash in `rustdesk-1.4.6-vendor-staging`, surfaces as a `rustdesk-org/wezterm` git-fetch failure). The fix is the version bump [PR #527831](https://github.com/NixOS/nixpkgs/pull/527831) (`1.4.6 → 1.4.7`), to be backported to 26.05. Modules ([modules/apps/linux/rustdesk.nix](modules/apps/linux/rustdesk.nix), [modules/apps/darwin/rustdesk.nix](modules/apps/darwin/rustdesk.nix)) and the HOME-MANAGER.md rows are kept intact — only the three Linux `custom.appRustdesk.enable = true;` lines are commented out. EDGE (Homebrew cask) is unaffected and stays enabled. Re-enable plan: `nix flake update`, confirm `nix build .#nixosConfigurations.JIN.config.system.build.toplevel` builds rustdesk, then uncomment the enable line on all three hosts.
