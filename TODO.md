...
- [x] **LAN IPv4 addresses exposed across all metadata** — Every `hosts/*/metadata.nix` encodes a real internal IP (JIN `192.168.1.10`, EDGE `192.168.1.50`, YUNA `192.168.40.2`, SLIME `192.168.60.3`, LaJuvePixel6 `192.168.1.30`, LaJuvePixel9 `192.168.1.31`, plus FENNEC) plus Syncthing peer addresses inside `syncthing.addresses`. Together they reveal full internal topology including VLAN segmentation (`192.168.40.*`, `192.168.60.*`). **Fix:** split each `metadata.nix` into a public stub (hostname, managed, os, optional syncthing.id) + a gitignored `metadata.local.nix` (or `secrets/<host>.nix`) holding `lanIp` + `syncthing.addresses`. Update `flake/metadata.nix` to merge optionally (`builtins.pathExists`-guarded import). LULA already ships `lanIp = null` — use it as the shape exemplar. **Resolved:** bundled with P1 #3 below. `lanIp` was removed entirely (zero code consumers in the repo; can be re-added when a real consumer appears). `syncthing.addresses` moved to the sibling `nix-config-private` repo together with `syncthing.id` (split `id` from `addresses` would have created awkward merge semantics). Public stubs now hold only `hostname`/`managed`/`os`.
- [x] **Syncthing device IDs are persistent device fingerprints** — Five IDs in tree: EDGE (`57HA3EL-…-LYNOMAX`), FENNEC (`OPSABBX-…-XM5ZYAB`), YUNA (`E25DNYB-…-KFNU5AL`), SLIME (`R6BJ3B2-…-SXWD5AJ`), LaJuvePixel9 (`E2ZNFF2-…-NWJLCAU`). Once published they are forever a stable identifier mapping the repo to specific physical machines via global Syncthing discovery. **Fix:** move IDs into the same gitignored split as `lanIp`. **Mitigation:** anything already pushed is exfiltrated — regenerate each device's Syncthing identity (delete `cert.pem`+`key.pem`, restart) after the split lands, then update the new local files. Rotation disrupts every peer (re-pair on each device); schedule for a quiet evening. **Resolved:** P1 #2 + #3 implemented together as a two-repo split (public `nix-config` + sibling private `nix-config-private`, layout mirroring the public `hosts/` structure). `flake/metadata.nix` now does a `pathExists`-guarded `lib.recursiveUpdate` merge — public repo builds with or without the sibling. Bootstrap is `gh repo clone elvismercado/nix-config-private` next to the public repo. A multi-root VS Code workspace (`nix-config.code-workspace`) opens both folders in one window. Forward-looking only — the already-exfiltrated IDs still need rotation; tracked as a Backlog item.

### P2 — Robustness & Reliability (MEDIUM)

- [x] **Real first names baked into usernames + comments** — `hosts/EDGE/user-settings.nix` (`username = "elvis"`), `hosts/LULA/user-settings.nix` (`username = "maria"`), and "Maria's …" descriptors in `hosts/LULA/metadata.nix` header + `hosts/LULA/README.md`. JIN/FENNEC use pet-style names and are unaffected. **Fix:** decision required. Renaming a macOS user is non-trivial (`/Users/<old>` won't auto-rename, Brave Sync / iCloud / Homebrew prefixes break). Two options — (a) keep usernames, strip "Maria's …" descriptors in favour of role wording ("primary user", "secondary daily-driver"); (b) plan a coordinated rename to neutral handles (`edge`, `lula`) at the next OS reinstall. Recommend (a) for now; revisit (b) for LULA before the device arrives so it ships clean. **Resolved:** applied option (b) for LULA (pre-delivery, no live `/Users/maria` to migrate) bundled with option (a)'s prose cleanup. Renamed `username` from `"maria"` to `"lula"` in `hosts/LULA/user-settings.nix` (propagates to `users.users.${username}`, `system.primaryUser`, `users.knownUsers`, and `git.nix`'s `user.email` derivation). Stripped "Maria"/"she" descriptors from `hosts/LULA/metadata.nix` header, `README.md` (intro + Brave + AppCleaner bullets), `configuration/configuration.nix` (SoftwareUpdate comment), and `configuration/default.nix` (Homebrew comment). Updated `DARWIN.md` host table User column. EDGE `username = "elvis"` kept as-is (accepted disclosure — already implied by repo owner `elvismercado`).
- [x] **Disk / swap UUIDs in `hardware-configuration.nix`** — `hosts/JIN/configuration/hardware-configuration.nix` and `hosts/FENNEC/configuration/hardware-configuration.nix` use `by-uuid` paths for `/`, `/boot`, swap. NixOS effectively requires these for `fileSystems.*.device`; flagging because they uniquely identify the physical drives and tie commits to a specific machine. **Fix:** no action — accept as required exposure. Add a one-line header note to each `hardware-configuration.nix` documenting the accepted-disclosure decision so future audits don't re-flag. Spot-check that no UUID appears outside `hardware-configuration.nix` (none today; re-verify when P1 split lands). **Resolved:** appended a `# PII note:` paragraph inside the existing `nixos-generate-config` warning block of both files (JIN + FENNEC), explaining `by-uuid` is mandatory for `fileSystems.*.device` and that the note must be re-added after a fresh `nixos-generate-config` run (script-level enforcement deferred). Spot-checked: all 7 UUIDs (3 in JIN, 4 in FENNEC including its `/home`) appear only inside the two `hardware-configuration.nix` files — zero leakage elsewhere in the tree. No semantic change; function args / imports / kernelModules / fileSystems / swapDevices / hostPlatform / microcode all byte-identical.
- [x] **Hardware fingerprint detail in per-host READMEs** — `hosts/JIN/README.md`, `hosts/FENNEC/README.md`, `hosts/EDGE/README.md`, `hosts/LULA/README.md` document exact RAM model (e.g. Corsair CMK32GX4M2B3200C16), specific SSD model+capacity, GPU+VRAM, monitor firmware revision. Combined with timezone and GitHub handle this narrows physical-machine identification. **Fix:** decision per host. Recommend keeping JIN/FENNEC verbose (desktops, rarely visible publicly) and trimming EDGE/LULA to generic class strings ("32 GB DDR4-3200", "1 TB NVMe SSD", "RTX 3080-class") — they're laptops, travel-photographable. **Resolved:** trimmed only the `Apple SSD AP1024M` row in EDGE's Hardware table to `1 TB NVMe SSD` (the one cell that reads like a serial-adjacent identifier). Declined the broader trim: threat model is weak (anyone seeing the laptop can read its "About" pane / `system_profiler` output, and EDGE's H1 subtitle already names model year + 15" + Core i9 — the same fingerprint as the table); broader trim costs documentation value for the legitimate audience (the maintainer); and trimming EDGE+LULA while keeping JIN/FENNEC verbose by design would be internally inconsistent. LULA Hardware table left as-is (already generic with `(fill in after delivery)` placeholders). JIN/FENNEC kept verbose by design.

### P3 — Architecture & Convention (LOW)

- [x] **Timezone `Europe/Amsterdam` in every `user-settings.nix`** — Broad regional disclosure (CET/CEST spans ~25 countries). Acceptable in isolation; flagged for completeness. **Fix:** no action; record as accepted exposure so future audits don't re-surface. **Resolved:** went beyond the original "no action" with a structural fix instead. (a) Added an `Etc/UTC` fallback inline in `modules/systems/nixos/system/time.nix` via `userSettings.timeZone or "Etc/UTC"` (matches the established `desktopEnvironment or null` pattern, 8 existing instances). (b) Created `modules/systems/darwin/time.nix` with `custom.sysDarTimezone.enable` closing the darwin gap (nix-darwin does support `time.timeZone`, the module was just missing). (c) Extended `flake/hosts.nix` `mkHost` with a `pathExists`-guarded private overlay merge (`publicUserSettings // privateUserSettings`); no value defaults live in the builder. (d) Created `nix-config-private/hosts/{JIN,FENNEC,EDGE,LULA}/user-settings.nix` each holding the host's `timeZone`. Public `hosts/*/user-settings.nix` no longer carry the value; EDGE and LULA wire the new darwin module via `custom.sysDarTimezone.enable = true;`.
- [x] **Cloud-service app intent (no credentials)** — Modules enabling Nextcloud, Insync (Google Drive), Proton Mail Bridge, Mullvad VPN, Syncthing reveal which third-party services the user has accounts with. No URLs / tokens / account hints are committed; the inference is purely "user has these accounts". **Fix:** no action — these modules ARE the install intent. Documented as accepted exposure. **Resolved:** logged as accepted exposure with no code change. The five app façades (`appNextcloud`, `appInsync`, `appProtonmailBridge`, `appMullvadVpn`, `appSyncthing`) stay enabled on their respective hosts — they exist precisely to install the apps the user uses, so removing/renaming them would defeat the purpose. Reader inference is bounded to "user has accounts with these services"; no service URLs, account IDs, tokens, server hostnames, or device IDs sit in the public tree. Syncthing device IDs (the only one of these with persistent-fingerprint potential) already moved to `nix-config-private` via the Round 15 P1 #2/#3 metadata split. The cross-cutting "what's accepted exposure" prose belongs in the P3 #4 backstop work and should cross-link this note rather than re-listing services.
- [x] **Git history may contain pre-cleanup leaks** — Out of scope for working-tree audits; commit history and old refs can contain values deleted from `main`. **Fix:** before the next public push (or before applying the P1 metadata split), run locally: `git log --all -S "192.168" --oneline`, `git log --all -S "<one syncthing ID prefix>" --oneline`, `git log --all --grep -iE "password|secret|token|credential"`. If hits exist in old commits, evaluate `git filter-repo` rewrite vs accept-and-rotate. **Mitigation:** any IP / Syncthing ID already on `main` is already public regardless of cleanup — rotation is the only real fix. **Resolved:** the three `git log` probes above ARE the close-out — they stay documented inline in this entry and are treated as the pre-push hygiene checklist for future-self (no separate artifact, no `pre-push` hook — single-maintainer repo, over-engineering avoided). `git filter-repo` rewrite **declined**: force-pushing rewritten history doesn't unpublish anything already cloned/cached/archived, breaks downstream forks/clones, and gives a false sense of security. For already-exposed Syncthing device IDs the real mitigation is rotation — see Backlog → Round 15 follow-ups → "Rotate already-exfiltrated Syncthing device IDs" for the per-device procedure. For LAN IPs / hostnames already on `main`, accepted as published (bounded exposure inside RFC1918). The P3 #4 README backstop should cross-link this entry so the checklist surfaces alongside the no-secrets-in-tree rule.
- [x] **No backstop for future contributors / future-self** — No `CONTRIBUTING.md` / `SECURITY.md` / repo-level note describing the "no secrets in tree" rule, the `*.local.nix` split, or the pre-push checklist. **Fix:** after the P1 items are resolved, add a short section in `README.md` (or a header comment in each `metadata.nix` template) pointing at the local-file split and the pre-push `git log -S` check. Keep it lightweight — single-maintainer repo. **Resolved:** added a `## PII & Secrets Discipline` section to `README.md` (between "Private Sibling Repo" and "Per-Host Settings"). Four short subsections: the split, `.gitignore` guards, accepted exposures, and the pre-push checklist. Cross-links into this TODO file (P3 #2 reasoning, P3 #3 rationale, Backlog rotation procedure) keep the section thin and avoid drift. `SECURITY.md` / `CONTRIBUTING.md` declined — those signal an external-contributor workflow that doesn't exist for this repo. Per-stub `metadata.nix` header comments declined — scattering the same rule across 8 stubs is worse than one README place.
...
- [ ] **Rotate already-exfiltrated Syncthing device IDs.** The two-repo split (P1 #2 + #3) is forward-looking — the IDs already on `main` (EDGE `57HA3EL-…`, FENNEC `OPSABBX-…`, YUNA `E25DNYB-…`, SLIME `R6BJ3B2-…`, LaJuvePixel9 `E2ZNFF2-…`) remain valid persistent device fingerprints in global Syncthing discovery until rotated. **Procedure per device:** stop Syncthing → delete `cert.pem` + `key.pem` from the config dir → restart → capture the new ID from the Web UI → update `nix-config-private/hosts/<HOST>/metadata.nix` → re-pair from every peer. Rotation disrupts every peer simultaneously, so do all five in one sitting. Schedule for a quiet evening. **Note:** repo's own git history (the rotation removes the IDs from future eval, but they stay in the commit log) — if the original push history is also a concern, evaluate `git filter-repo` separately (see P3 #3 in Round 15).
