...
- [ ] **`flake/metadata.nix` cannot load the private sibling during normal flake evaluation.** [flake/metadata.nix](flake/metadata.nix#L64) uses `privateMetadata = ../../infra-nix-config-private/metadata.nix;`, but Nix evaluates the flake from its copied `/nix/store/...-source` tree. The relative path therefore cannot reach the live sibling and `private` falls back to `{ }`, leaving Syncthing metadata and private-only peers unavailable. Pass `inputs.private` into the metadata loader and read `inputs.private + "/metadata.nix"` so metadata uses the same pinned source as [flake/hosts.nix](flake/hosts.nix#L17). Keep this with the offline-stub decision below rather than changing the companion architecture during the repository rename.
