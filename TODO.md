...
- [ ] **`.gitignore` has no guard against accidental secret commits** — `.gitignore` currently only contains `**/INSTALL-REPORT.md`. Any future `*.local.nix`, `secrets/`, `.env`, agenix `*.age`, or sops `*.yaml` would be committed by default. **Fix:** before introducing the metadata split below, add explicit guard patterns: `*.local.nix`, `secrets/`, `.env`, `.env.*`, `*.age`, `*.gpg`. Add a `!*.example.local.nix` allow-line so committed templates stay tracked.
